...
def get_indice_pairs(indices: torch.Tensor,
    ...
    if (not subm) and (not spss) and (not focal) and (not sprs):
        if transpose:
    ...
    if spss or sprs:
        assert mask is not None, "mask could not be None in SPSS or SPRS"
        mask_tv = torch_tensor_to_tv(mask.int())

    if focal:
        assert ori_feat_num>0, "ori_feat_num must be positive in FocalsConv"

    if subm or spss or focal or sprs:
        out_inds = indices
        # out_inds = indices if not spss else indices[mask]
        if indices.is_cuda:
            stream = get_current_stream()
            ...
            if subm:
                if ksize_ori[0] > 0:
                    assert not group_map is None, "group_map can not be none when use spatial-wise group conv"
                    group_map_tv = torch_tensor_to_tv(group_map)
                    max_group_item_num = int(group_map.shape[-1])
                    pair = torch.full((3, kv, indices.shape[0]*max_group_item_num), -1, dtype=indices.dtype, device=indices.device)
                    pair_tv = torch_tensor_to_tv(pair)

                    SpconvOps.generate_spatial_group_conv_inds(inds_tv,
                                                    hashdata_tv,
                                                    pair_tv,
                                                    out_inds_tv,
                                                    ...
                                                    ksize=ksize,
                                                    ksize_ori=ksize_ori,
                                                    max_group_item_num=max_group_item_num,
                                                    group_map=group_map_tv,
                                                    dilation=dilation,
                                                    stream_int=stream)
                    pair = pair[:, :, :indice_num_per_loc.max()].contiguous()
                    #print('pair', pair)
                ...
                SpconvOps.generate_spss_conv_inds(inds_tv,
                                                hashdata_tv,
                                                pair_tv,
                                                out_inds_tv,
                                                indice_num_per_loc_tv,
                ...
                SpconvOps.generate_focal_conv_inds(inds_tv,
                                                hashdata_tv,
                                                pair_tv,
                                                out_inds_tv,
                                                indice_num_per_loc_tv,
                                                ...
                                                dilation=dilation,
                                                stream_int=stream,
                                                ori_feat_num=ori_feat_num)
        else:
            assert not (spss and focal), "Not support cpu in SPSS and FocalsConv"
            out_inds_tv = torch_tensor_to_tv(out_inds)
        ...
        if indices.is_cuda:
            ...
            assert not (sprs), "Not support cpu in SPRS"
...
def indice_conv(features: torch.Tensor,
    ...
    inited: bool = subm
    # print("a-features shape:", features.shape, "c-features shape:", out_features.shape)
    ...
    if tuned_res is None:
        ...
        # print("profile_idx:", profile_idx, "nhot_profile:", nhot_profile)
        out = torch.unique(inp_indices_th)
        # print("out shape:", out.shape)
        # print("inp_indices_th shape:", inp_indices_th[0:10])
        out_indices_th = indice_pairs[int(not inverse)][
            profile_idx, :nhot_profile]
        # print("out_indices_th shape:", out_indices_th[0:10])
...
def indice_conv_spatial_groups(features: torch.Tensor,
                filters: torch.Tensor,
                indice_pairs: torch.Tensor,
                indice_pair_num: torch.Tensor,
                num_activate_out: int,
    ...
    #_inv_list, indice_pairs_groups, indice_pair_num_groups = _reform_spatial_groups(num_features, indice_pairs, indice_pair_num)
    ...
    inited: bool = subm
    a = torch_tensor_to_tv(features)
    #a_groups = torch_tensor_to_tv(features_groups)
    c = torch_tensor_to_tv(out_features)
    indice_pairs_tv = torch_tensor_to_tv(indice_pairs)
    pair_in = indice_pairs_tv[int(inverse)]
    pair_out = indice_pairs_tv[int(not inverse)]
    ...
    with timer.record("forward", stream):
        for i, nhot in enumerate(indice_pair_num_cpu):
            ...
            features_groups = features.new_zeros((num_features, features.shape[-1]), requires_grad=True)
            indices_in = indice_pairs[0, i, :nhot]
            _features = features[indices_in.long()]

            indices_out = indice_pairs[1, i, :nhot]
            indices_unique, _inv = indices_out.unique(return_inverse=True)
            if not position_embedding is None:
                map_to_ori = indice_pairs[2][i, :nhot].long()
                _features += position_embedding[map_to_ori]
            features_groups.index_add_(0, _inv, _features)
            a = torch_tensor_to_tv(features_groups)

            inp_indices = torch_tensor_to_tv(torch.arange(int(indices_unique.shape[0]), dtype=torch.int32, device=features.device))
            out_indices = torch_tensor_to_tv(indices_unique)
...
def group_mult(features, filters, groups, concat=False):
    ...
    out_features_batch = torch.matmul(features_batch, filters_batch)
    out_features = [out_f for out_f in out_features_batch]
    if concat:
        out_features = torch.cat(out_features, dim=-1)
    return out_features

def indice_conv_groups(features: torch.Tensor,
                filters: torch.Tensor,
                indice_pairs: torch.Tensor,
                indice_pair_num: torch.Tensor,
                num_activate_out: int,
    ...
    if features.dtype == torch.int8 or features.dtype == torch.qint8:
        raise NotImplementedError("work in progress")
    if FILTER_HWIO:
        out_channel = filters.shape[-1]//groups
    else:
        out_channel = filters.shape[-2]//groups
    filters = filters.reshape(-1, *filters.shape[-2:])
    kv = filters.shape[0]
    kv_center = kv // 2
    if subm:
        ...
        out_features = [torch.zeros((num_activate_out, out_channel),
                                   dtype=features.dtype,
                                   device=features.device) for _ in range(groups)]
    if kv == 1 and subm:
        return out_features
    ...
    features_batch = features.chunk(groups, dim=1)
    filters_batch = filters.chunk(groups, dim=2 if FILTER_HWIO else 1)

    inited: bool = subm
    # print("a-features shape:", features.shape, "c-features shape:", out_features.shape)
    a_list = [torch_tensor_to_tv(f.contiguous()) for f in features_batch]
    c_list = [torch_tensor_to_tv(f) for f in out_features]
    indice_pairs_tv = torch_tensor_to_tv(indice_pairs)
    pair_in = indice_pairs_tv[int(inverse)]
    pair_out = indice_pairs_tv[int(not inverse)]
    filters_tv_list = [torch_tensor_to_tv(f.contiguous()) for f in filters_batch]
    #filters_tv_list2 = [torch_tensor_to_tv(f.clone()) for f in filters_batch]
    ...
    if not features.is_cuda:
        ...
        assert not filters.is_cuda
        assert not indice_pairs.is_cuda
        inp_buffer = torch.empty([maxnhot, features.shape[1]], dtype=features.dtype)
        out_buffer = [torch.empty([maxnhot, out_features[i].shape[1]],
                                 dtype=out_features[i].dtype) for i in range(groups)]
        inp_buffer_tv = torch_tensor_to_tv(inp_buffer) #[torch_tensor_to_tv(_buffer) for _buffer in inp_buffer]
        out_buffer_tv = [torch_tensor_to_tv(_buffer) for _buffer in out_buffer]
        a = torch_tensor_to_tv(features.contiguous())
        ...
        for i, nhot in enumerate(indice_pair_num_cpu):
            ...
            #torch.mm(inp_buffer[:nhot], filters_cur, out=out_buffer[:nhot])
            out_buffer_list = group_mult(inp_buffer[:nhot], filters_cur, groups)

            for g in range(groups):
                out_buffer[g][:nhot] = out_buffer_list[g]
                SpconvOps.scatter_add_cpu(c_list[g], out_buffer_tv[g], out_indices)
        out_features = torch.cat(out_features, dim=-1)
        return out_features
    stream = get_current_stream()
    ...
    for ind in range(groups):
        tuned_res = GEMM.get_tuned_algo(a_list[ind].dtype,
                                        filters_tv_list[ind].dtype,
                                        c_list[ind].dtype,
                                        a_list[ind].shape,
                                        filters_batch[ind].shape[-2:],
                                        c_list[ind].shape,
                                        False,
                                        False if FILTER_HWIO else True,
                                        False,
        ...
        if tuned_res is None:
            ...
            inp_indices = torch_tensor_to_tv(inp_indices_th)
            out_indices = torch_tensor_to_tv(out_indices_th)
            filters_tv_ = torch_tensor_to_tv(filters_batch[ind].contiguous())[profile_idx]
        ...
        # t = time.time()
        with timer.record("forward", stream):
            #filters_tv = torch_tensor_to_tv(filters_batch[ind].contiguous()) #filters_tv_list[ind]
            #a = torch_tensor_to_tv(features_batch[ind].contiguous())
            #c = torch_tensor_to_tv(out_features[ind].contiguous())
            for i, nhot in enumerate(indice_pair_num_cpu):
                #print('i, nhot', i, nhot)
                #print('ind', ind)
                if subm and i == kv_center:
                ...
                #torch.cuda.synchronize()
                #print('ind', ind)
                #print('features_batch[ind]', features_batch[ind])
                #print('filters_batch[ind]', filters_batch[ind])
                #print(filters_batch[ind])
                indice_pairs_tv = torch_tensor_to_tv(indice_pairs)
                pair_in = indice_pairs_tv[int(inverse)]
                pair_out = indice_pairs_tv[int(not inverse)]
                ...
                a = torch_tensor_to_tv(features_batch[ind].contiguous())
                #c = torch_tensor_to_tv(out_features[ind].contiguous())
                b = torch_tensor_to_tv(filters_batch[ind].contiguous())[i] #filters_tv[i]
                #b = filters_tv[i]
                # inp @ filter.T, NC @ KC
                beta = 1.0 if inited else 0.0
                algo_desp = GEMM.run_with_tuned_result(
...
def indice_conv_backward(features: torch.Tensor,
    ...
    if subm:
        return (din, dfilters.reshape(filters_shape), None)
    return (din, dfilters.reshape(filters_shape))


def indice_conv_backward_spatial_groups(features: torch.Tensor,
                         filters: torch.Tensor,
                         out_bp: torch.Tensor,
                         indice_pairs: torch.Tensor,
                         indice_pair_num: torch.Tensor,
    ...
    # t = time.time()
    inited = subm
    dpe = torch.zeros_like(position_embedding, device=position_embedding.device)
    ...
    for i, nhot in enumerate(indice_pair_num_cpu):
        ...
        map_to_ori = indice_pairs[2][i, :nhot].long()
        indices_in = indice_pairs[0, i, :nhot]
        _din = din[indices_in.long()]
        dpe.index_add_(0, map_to_ori, _din)
    # torch.cuda.synchronize()
    # dw_time = time.time() - t
    # # print(dw_time + di_time, di_time, dw_time, tuned_res_wgrad.splitk, tuned_res_wgrad.algo_desp, dfilters.shape)
    # # print(dw_time + di_time)
    # print("BWG", time.time() - t)
    return (din, dfilters.reshape(filters_shape), dpe)
...
def indice_maxpool(features: torch.Tensor, indice_pairs: torch.Tensor,
                   indice_pair_num: torch.Tensor, num_activate_out, subm=False, return_inverse=True):
    # torch.cuda.synchronize()
    # t = time.time()
    # stream = get_current_stream()
    # CONV.stream_synchronize(stream)
    ...
    inverse = torch.arange(features.shape[0], device=features.device, dtype=torch.int32).unsqueeze(-1).repeat(1, features.shape[-1])
    if subm:
    ...
    features_tv = torch_tensor_to_tv(features)
    indice_pairs_tv = torch_tensor_to_tv(indice_pairs)
    inverse_tv = torch_tensor_to_tv(inverse)

    kv = len(indice_pair_num_cpu)
    kv_center = kv // 2
    for i, nhot in enumerate(indice_pair_num_cpu):
        if subm and i == kv_center:
            continue
        ...
        if is_cpu:
            ...
            SpconvOps.maxpool_forward(out_features_tv, features_tv,
                                      out_indices, inp_indices, inverse_tv, stream)

    # CONV.stream_synchronize(stream)
    # print("M", time.time() - t)

    return out_features, inverse


def indice_maxpool_backward(features, out_features, out_bp, indice_pairs,
                            indice_pair_num, subm=False):
    out_channel = features.shape[-1]
    din = torch.zeros_like(features)
    is_cpu = not features.is_cuda
    stream = 0
...
def indice_avgpool(features: torch.Tensor, indice_pairs: torch.Tensor,
                   indice_pair_num: torch.Tensor, num_activate_out, subm=True):
    # torch.cuda.synchronize()
    # t = time.time()
    # stream = get_current_stream()
    # CONV.stream_synchronize(stream)
    ...
    out_features = torch.zeros((num_activate_out, out_channel),
                               dtype=features.dtype,
                               device=features.device)
    out_nums = torch.zeros((num_activate_out + 1, 1), dtype=torch.int, device=features.device)

    stream = 0
    is_cpu = not features.is_cuda
    assert not is_cpu, "CPU version average pool is not support for now"
    ...
    features_tv = torch_tensor_to_tv(features)
    indice_pairs_tv = torch_tensor_to_tv(indice_pairs)
    out_nums_tv = torch_tensor_to_tv(out_nums)
    kv = len(indice_pair_num_cpu)
    kv_center = kv // 2
    for i, nhot in enumerate(indice_pair_num_cpu):
        if subm and i == kv_center:
        ...
        inp_indices = indice_pairs_tv[0][i].slice_first_axis(0, nhot)
        out_indices = indice_pairs_tv[1][i].slice_first_axis(0, nhot)
        SpconvOps.avgpool_forward(out_features_tv, features_tv,
                                out_indices, inp_indices, out_nums_tv, stream)

    num_feature_groups = out_nums[-1]
    out_nums = (out_nums / num_feature_groups)[:-1]
    if subm:
        out_nums = out_nums + 1
        out_features = (out_features + features)

    #out_nums[:] = 3**3
    out_features /= out_nums
    # CONV.stream_synchronize(stream)
    # print("M", time.time() - t)

    return out_features, out_nums


def indice_avgpool_backward(features, out_features, out_bp, indice_pairs,
                            indice_pair_num, out_nums, subm=True):
    out_channel = features.shape[-1]
    din = torch.zeros_like(features)
    is_cpu = not features.is_cuda
    assert not is_cpu, "CPU version average pool is not support for now"
    ...
    for i, nhot in enumerate(indice_pair_num_cpu):
        ...
        SpconvOps.avgpool_backward(out_features_tv, features_tv, out_bp_tv, din_tv,
                                    out_indices, inp_indices, out_nums_tv, stream)

    #from IPython import embed; embed()
    #din /= num_feature_groups
    return din
