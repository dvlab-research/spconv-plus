...
class SparseMaxPool(SparseModule):
    ...
    def forward(self, input, return_inverse=False):
        ...
        with profile_ctx:
            if self.algo == ConvAlgo.Native:
                outids, indice_pairs, indice_pairs_num = ops.get_indice_pairs(
                    ...
                    self.kernel_size, [-1], self.stride, self.padding, self.dilation,
                    out_padding, self.subm)
                if input.benchmark:
                    torch.cuda.synchronize()
                ...
                out_features, inverse = Fsp.indice_maxpool(features,
                                                  indice_pairs.to(device),
                                                  indice_pairs_num.to(device),
                                                  outids.shape[0], self.subm, return_inverse)
            else:
        ...
        out_tensor.inverse = inverse if return_inverse else None
        return out_tensor
...
class SparseAvgPool(SparseModule):
    def __init__(self,
        ...
        if algo is None:
            # keep in mind that this algorithm is set for Inverse Sparse Conv
            # avgpool itself don't need mask.
            if kv <= 32 and not CPU_ONLY_BUILD:
                if kv < 8:
                    algo = ConvAlgo.MaskImplicitGemm
                else:
    ...
    def forward(self, input):
        ...
        if input._timer is not None and self._sparse_unique_name:
            profile_ctx = input._timer.namespace(self._sparse_unique_name)
        with profile_ctx:
            assert self.algo == ConvAlgo.Native, "algo only support Native for average pool for now."
            outids, indice_pairs, indice_pairs_num = ops.get_indice_pairs(
                indices, batch_size, spatial_shape, ConvAlgo.Native,
                self.kernel_size, [-1], self.stride, self.padding, self.dilation,
                out_padding, self.subm)
            ...
            out_features = Fsp.indice_avgpool(features,
                                                indice_pairs.to(device),
                                                indice_pairs_num.to(device),
                                                outids.shape[0], self.subm)
...
class SparseAvgPool1d(SparseAvgPool):
    def __init__(self,
                 ...
                 algo: Optional[ConvAlgo] = None,
                 name=None):
        super(SparseAvgPool1d, self).__init__(1,
                                              kernel_size,
                                              stride,
                                              padding,
                                              dilation,
...
class SparseAvgPool2d(SparseAvgPool):
    def __init__(self,
                 ...
                 algo: Optional[ConvAlgo] = None,
                 name=None):
        super(SparseAvgPool2d, self).__init__(2,
                                              kernel_size,
                                              stride,
                                              padding,
                                              dilation,
...
class SparseAvgPool3d(SparseAvgPool):
    def __init__(self,
                 ...
                 algo: Optional[ConvAlgo] = None,
                 name=None):
        super(SparseAvgPool3d, self).__init__(3,
                                              kernel_size,
                                              stride,
                                              padding,
                                              dilation,
...
class SparseAvgPool4d(SparseAvgPool):
    def __init__(self,
                 kernel_size,
                 stride=None,
                 padding=0,
                 ...
                 algo: Optional[ConvAlgo] = None,
                 name=None):
        super(SparseAvgPool4d, self).__init__(4,
                                              kernel_size,
                                              stride,
                                              padding,
                                              dilation,
