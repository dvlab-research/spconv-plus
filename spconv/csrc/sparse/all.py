...
from .avgpool import IndiceAvgPool
...
class SpconvOps(pccm.Class):
        ...
        for ndim in self.ndims:
            ...
            if not CUMM_CPU_ONLY_BUILD:
                ...
                cuda_funcs = [
                    ...
                    self.generate_spatial_group_conv_inds,
                    self.generate_spss_conv_inds,
                    self.generate_focal_conv_inds,
                    self.generate_sprs_conv_inds_stage1,
    ...
    def generate_sprs_conv_inds_stage1(self):
        ...
        for ndim in self.ndims:
            ...
            if (ndim == {ndim}){{
                ...
                return SpconvIndices{ndim}D::generate_sprs_conv_inds_stage1(indices, hashdata,
                    indice_pairs, indice_pairs_uniq, indice_num_per_loc, im_mask,
                    batch_size, output_dims_, input_dims_, 
                    ksize_, stride_, padding_, dilation_, transposed, stream_int);
            }}
            """)
    ...
    def generate_spatial_group_conv_inds(self):
        ...
        code.arg("batch_size", "int")
        code.arg("input_dims", f"std::vector<int>")
        code.arg("ksize, ksize_ori, dilation", f"std::vector<int>")
        ...
        for ndim in self.ndims:
            ...
            if (ndim == {ndim}){{
                tv::array<int, {ndim}> input_dims_;
                tv::array<int, {ndim}> ksize_, ksize_ori_, dilation_;
                for (int i = 0; i < {ndim}; ++i){{
                    input_dims_[i] = input_dims[i];
                    ksize_[i] = ksize[i];
                    ksize_ori_[i] = ksize_ori[i];
                    dilation_[i] = dilation[i];
                }}
                return SpconvIndices{ndim}D::generate_spatial_group_conv_inds(indices, hashdata,
                    indice_pairs, out_inds, indice_num_per_loc,
                    batch_size, input_dims_, 
                    ksize_, ksize_ori_, dilation_, group_map, max_group_item_num,
                     indice_pair_mask, backward, stream_int);
            }}
            """)
        code.raw(f"""TV_THROW_RT_ERR("unknown ndim", ndim);""")
        return code.ret("int")
    ...
    def generate_spss_conv_inds(self):
        ...
        for ndim in self.ndims:
            ...
            if (ndim == {ndim}){{
                ...
                return SpconvIndices{ndim}D::generate_spss_conv_inds(indices, hashdata,
                    indice_pairs, out_inds, indice_num_per_loc,
                    im_mask, batch_size, input_dims_, 
                    ksize_, dilation_, indice_pair_mask, backward,
                    stream_int);
            }}
            """)
    ...
    def generate_focal_conv_inds(self):
        ...
        for ndim in self.ndims:
            ...
            if (ndim == {ndim}){{
                ...
                return SpconvIndices{ndim}D::generate_focal_conv_inds(indices, hashdata,
                    indice_pairs, out_inds, indice_num_per_loc,
                    batch_size, input_dims_, ksize_, dilation_, 
                    indice_pair_mask, backward, stream_int, ori_feat_num);
            }}
            """)
        code.raw(f"""TV_THROW_RT_ERR("unknown ndim", ndim);""")
        return code.ret("int")
    ...
    def maxpool_forward(self):
        ...
        code.arg("inverse", "tv::Tensor")
        code.arg("stream", "std::uintptr_t", "0", pyanno="int")
        code.add_dependency(IndiceMaxPool)
        code.raw(f"""
        return IndiceMaxPool::forward(out, inp, out_inds, in_inds, inverse, stream);
        """)
        return code
    
    @pccm.pybind.mark
    @pccm.cuda.static_function
    def avgpool_forward(self):
        code = pccm.FunctionCode()
        if CUMM_CPU_ONLY_BUILD:
            return code.make_invalid()
        code.arg("out", "tv::Tensor")
        ...
        code.arg("stream", "std::uintptr_t", "0", pyanno="int")
        code.add_dependency(IndiceAvgPool)
        code.raw(f"""
        return IndiceAvgPool::forward(out, inp, out_inds, in_inds, out_nums, stream);
        """)
        return code
    ...
    def avgpool_backward(self):
        ...
        code.add_dependency(IndiceAvgPool)
        code.raw(f"""
        return IndiceAvgPool::backward(out, inp, dout, dinp, out_inds, in_inds, out_nums, stream);
        """)
        return code
